/*
  ...
  1. New Functions
    - `create_demo_admin()` - Creates the demo admin user with proper authentication and profile setup
  
  2. Demo User Setup
    - Creates admin@heven.com user in auth.users
    - Creates corresponding profile in profiles table
    - Creates admin entry in admin_users table
    - Sets up proper permissions and active status
  
  3. Security
    - Ensures RLS policies work correctly for admin users
